...
class VisionControl(QThread):
    def __init__(self):
        ...
        self.OptFlowMode = True
        self.YOLOMode = False
        self.YOLO_OptFlowMode = False
        ...
        self.e_y_integral = 0.0
        self.AUTO_Switch = 0
        self.Vision_Switch = 1
        self.roll, self.pitch, self.throttle, self.yaw_rate = 0, 0, 0, 0
        self.frame = None
        ...
        if pygame.joystick.get_count() == 0:
            ...
            self.radio = pygame.joystick.Joystick(JOY_DEVICE_ID)
            self.radio.init()
            print(f"Connected to: {self.radio.get_name()}")


        try:
            self.ser = serial.Serial(port=PORT, baudrate=BAUD, timeout=2)
            self.CRSF_initialized = True
        except Exception as e:
            print(f"Serial Error: {e}")
            self.CRSF_initialized = False
    ...
    def getFrame(self, frame):
        self.frame = frame
    def selectedpoint(self, event):
        self.select_point(cv2.EVENT_LBUTTONDOWN,event["x"], event["y"], 0, 0)
    ...
    def run(self):
        while True:
            ...
            if self.radio_initialized:
                ...
                axes = [self.radio.get_axis(i) for i in range(self.radio.get_numaxes())]

                self.roll     = int(axes[0]*RC_STICK_RANGE/2) + RC_STICK_MID
                self.pitch    = int(axes[1]*RC_STICK_RANGE/2) + RC_STICK_MID
                self.throttle = int(axes[2]*RC_STICK_RANGE/2) + RC_STICK_MID
                self.yaw_rate = int(axes[3]*RC_STICK_RANGE/2) + RC_STICK_MID

                # Read Buttons (Switches mapped as buttons)
                self.buttons = [self.radio.get_button(i) for i in range(self.radio.get_numbuttons())]
                self.AUTO_Switch = self.buttons[2]
                self.Vision_Switch = self.buttons[0]
            ...
            if self.frame is None:
                continue
            cv2.resize(self.frame, (FRAME_WIDTH, FRAME_HEIGHT), cv2.INTER_AREA)
            # ----- If AUTO mode is on -----
            if self.Vision_Switch>0:
                # -------- Get target ---------
                if self.OptFlowMode:
                    self.cx, self.cy = self.calcOptFlow(self.frame)
                elif self.YOLOMode:
                    pass
                    # cx, cy = get_YOLO_centroid(frame)
                elif self.YOLO_OptFlowMode:
                    self.cx, self.cy = -1, -1

                if self.AUTO_Switch>0:
                    controller_on = True
                    
                    if self.cx != -1 and self.cy != -1:
                        e_x = self.cx - FRAME_WIDTH / 2
                        e_x /= (FRAME_WIDTH / 2)
                        e_y = self.cy - FRAME_HEIGHT / 2
                        e_y /= (FRAME_HEIGHT / 2)
                        roll, pitch, throttle, yaw_rate = self.controller(e_x, e_y)
                else:
                    controller_on = False
            ...
            if controller_on:
                ...
                cv2.putText(self.frame, "MANUAL", (TextBox_x, TextBox_y), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 0, 255), thickness)

            if self.Vision_Switch>0:
                cv2.putText(self.frame, "Vision ON", (TextBox_x, int(TextBox_y + 1.3*TextHeight)), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 255, 0), thickness)
            else:
                cv2.putText(self.frame, "Vision OFF", (TextBox_x, int(TextBox_y + 1.3*TextHeight)), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 255, 0), thickness)
            ...
            # cv2.rectangle(self.frame, (int(H_CENTER-H_SPACE-SIZE_HALF), int(V_CENTER-SIZE_HALF)), (int(H_CENTER-H_SPACE+SIZE_HALF), int(V_CENTER+SIZE_HALF)), (200, 200, 200), 2) # center point
            # cv2.rectangle(self.frame, (int(H_CENTER+H_SPACE-SIZE_HALF), int(V_CENTER-SIZE_HALF)), (int(H_CENTER+H_SPACE+SIZE_HALF), int(V_CENTER+SIZE_HALF)), (200, 200, 200), 2) # center point
            
            # cv2.circle(self.frame, (int(H_CENTER-H_SPACE),                                              int(V_CENTER)                           ),5, (150, 150, 150), -1) # center point
            # cv2.circle(self.frame, (int(H_CENTER-H_SPACE+SIZE*(yaw_rate-RC_STICK_MID)/RC_STICK_RANGE),  int(V_CENTER-SIZE*(throttle-RC_STICK_MID)/RC_STICK_RANGE)),5, (255, 0, 0), -1) # center point
            # cv2.circle(self.frame, (int(H_CENTER+H_SPACE),                                              int(V_CENTER)                           ),5, (150, 150, 150), -1) # center point
            # cv2.circle(self.frame, (int(H_CENTER+H_SPACE+SIZE*(roll-RC_STICK_MID)/RC_STICK_RANGE),      int(V_CENTER-SIZE*(pitch-RC_STICK_MID)/RC_STICK_RANGE)   ),5, (255, 0, 0), -1) # center point
            
            if self.point_selected:
                cv2.circle(self.frame, (int(self.selected_point[0]), int(self.selected_point[1])), 5, (150, 150, 150), -1) # center point
                cv2.circle(self.frame, (int(self.cx), int(self.cy)), 5, (255, 0, 0), -1) # center point
...
# Ideally Use LOS instead of pixel errors directly
 ...
 # Portion of throttle range to hover at level flight
